...
class AzureDeploymentAgent(BaseAgentExecutorDeliveryAgent):
    """Deploy the generated project through the Codex Deployment Agent."""

    descriptor = AgentDescriptor(
        agent_id="deployment-agent",
        name="Deployment Agent",
        runtime="L4 LangGraph Agent Executor + L6 Codex Deployment Agent",
        stage="deployment",
    )
    ...
    def run(self, state: DeliveryState) -> DeliveryState:
        return run_deployment_agent_graph(
            state,
            runner=self.runner,
