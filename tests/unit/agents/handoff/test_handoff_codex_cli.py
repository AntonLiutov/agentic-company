...
def test_handoff_codex_runner_accepts_contract_artifacts(tmp_path):
    ...
    assert result.output_artifacts == [paths.html]
...
def test_handoff_codex_runner_recovers_contract_from_generated_project(tmp_path):
    ...
    def executor(command, prompt, timeout_seconds, log_path, raw_events_path):
        (target_dir / paths.html).parent.mkdir(parents=True)
...
def test_handoff_codex_prompt_is_agent_owned_and_non_exhaustive(tmp_path):
    ...
    assert "one client-facing HTML report" in prompt
    assert "high-level" in prompt
    assert "business-friendly" in prompt
    assert "user-friendly information" in prompt
    assert "internal file paths" in prompt
    assert "Treat raw artifact paths from the output contract" in prompt
    assert "Do not write a Markdown handoff summary" in prompt
    assert "Do not write a JSON handoff evidence file" in prompt
